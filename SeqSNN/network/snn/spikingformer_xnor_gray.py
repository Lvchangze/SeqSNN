...
class ConvPE(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000, num_steps=4):
        ...
        self.rpe_lif = neuron.LIFNode(tau = tau, detach_reset=detach_reset, step_mode="m", surrogate_function=surrogate.ATan())
...
class SSA(nn.Module):
    def __init__(self, length, tau, common_thr, dim, heads=8, qkv_bias=False, qk_scale=0.25):
        ...
        self.attn_lif = neuron.LIFNode(tau=tau, detach_reset=detach_reset, surrogate_function=surrogate.ATan(), v_threshold=common_thr/2)
    ...
    def forward(self, x):
        ...
        q = q_m_out.reshape(T, B, L, self.heads, D // self.heads).permute(0, 1, 3, 2, 4).contiguous() # T, B, heads, L, D//heads
        ...
        k = k_m_out.reshape(T, B, L, self.heads, D // self.heads).permute(0, 1, 3, 2, 4).contiguous() # T, B, heads, L, D//heads
        ...
        q = generate_gray_code_matrix(q, num_bits=10) # T, B, heads, L, D//heads+10
        k = generate_gray_code_matrix(k, num_bits=10) # T, B, heads, L, D//heads+10
        ...
        # attn_min, _ = torch.min(attn.detach(), dim=1, keepdim=True)
        # attn = (attn - attn_min) * self.scale
...
@NETWORKS.register_module("Spikingformer_XNOR_Gray")
class Spikingformer_XNOR_Gray(nn.Module):
    def __init__(
            self,
            dim: int,
